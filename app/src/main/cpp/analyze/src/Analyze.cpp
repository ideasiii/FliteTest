...
namespace engtts
{
    ...
    std::string Analyze::utf8_remove_wide(const std::string &text)
    {
    ...
    LabelsPtr Analyze::AnalyzeTextFragment(const TextFragmentPtr &fragment)
    {
        ...
        cst_utterance *curr_utt = flite_synth_text(
                this->utf8_remove_wide(fragment->getText()).c_str(), curr_vox);
        //- call hook
        ...
        for (i = 0, segment = relation_head(utt_relation(curr_utt, "Segment")); segment; segment = item_next(
                segment), i++)
        {
            std::string labelData = create_label(segment);
            Label *tmpLabel (new HTKLabel(labelData));
...
#define strcpy_s strcpy
    ...
    static std::string create_label(cst_item *item)
    {
        ...
        /* load endtone */
        strcpy_s(endtone, ffeature_string(item,
        ...
        if (strcmp(seg_c, "pau") == 0)
        {
            ...
            if (item_next(item) != NULL)
            {
                ...
                lisp_total_phrases = ffeature_int(item,
            ...
            snprintf(label, MAXBUFLEN - 1,
                     ...
                     strcmp(seg_pp, "0") == 0 ? "x" : seg_pp, strcmp(seg_p, "0") == 0 ? "x" : seg_p, seg_c,
                     strcmp(seg_n, "0") == 0 ? "x" : seg_n, strcmp(seg_nn, "0") == 0 ? "x" : seg_nn,
                     ...
                     sub_phrases + 1, lisp_total_phrases - sub_phrases, endtone, ffeature_int(item,
        ...
        else
        {
            ...
            snprintf(label, MAXBUFLEN - 1,
                     ...
                     strcmp(seg_pp, "0") == 0 ? "x" : seg_pp, strcmp(seg_p, "0") == 0 ? "x" : seg_p, seg_c,
                     strcmp(seg_n, "0") == 0 ? "x" : seg_n, strcmp(seg_nn, "0") == 0 ? "x" : seg_nn, tmp1 + 1,
                     tmp2 - tmp1, ffeature_int(item, "R:SylStructure.parent.R:Syllable.p.stress"),
                     ...
                     ffeature_int(item, "R:SylStructure.parent.R:Syllable.accented"), tmp2, tmp3 + 1,
                     tmp4 - tmp3, ffeature_int(item, "R:SylStructure.parent.R:Syllable.syl_in") + 1,
                     ...
                     ffeature_string(item, "R:SylStructure.parent.R:Syllable.syl_vowel"),
                     ...
                     ffeature_string(item, "R:SylStructure.parent.parent.R:Word.gpos"), tmp4,
                     ...
                     ffeature_int(item, "R:SylStructure.parent.parent.R:Word.content_words_in") + 1,
                     ffeature_int(item, "R:SylStructure.parent.parent.R:Word.content_words_out") + 1,
                     ...
                     strcmp(endtone, "0") == 0 ? "NONE" : endtone, ffeature_int(item,
